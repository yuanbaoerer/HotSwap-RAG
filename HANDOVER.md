...
| 向量存储 | 50% | ChromaDB/Pinecone 完整，Milvus/FAISS 未实现 |
| LLM 提供商 | 100% | OpenAI, Anthropic, Ollama, OpenAI兼容 |
| 工厂函数 | 100% | parser_factory, store_factory, llm_factory |
| 文档管理 API | 100% | 上传、列表、删除已实现 |
| 知识库 API | 20% | 仅定义接口，CRUD 未实现 |
| RAG 问答 API | 10% | 仅占位符，核心逻辑未实现 |
| 配置管理 API | 50% | 查询可用，保存未持久化 |
...
| 文件位置 | 功能 | 当前状态 | 实现建议 |
|----------|------|----------|----------|
| `backend/api/routes/chat.py:35` | RAG 查询处理 | 返回硬编码占位符 | 实现检索+生成流程 |
| `backend/api/routes/chat.py:48` | 流式 RAG 查询 (SSE) | 返回固定字符串 | 实现 SSE 流式响应 |
| `backend/api/routes/documents.py:67` | 文档解析处理 | 仅标记 completed | 调用 Parser + VectorStore |

### 4.2 API 路由 - 中优先级

| 文件位置 | 功能 | 当前状态 | 实现建议 |
|----------|------|----------|----------|
| `backend/api/routes/knowledge_base.py:48` | 知识库列表 | 返回空列表 | 连接数据库查询 |
| `backend/api/routes/knowledge_base.py:55` | 创建知识库 | 返回 temp-id | 创建数据库记录 |
| `backend/api/routes/knowledge_base.py:74` | 获取知识库详情 | 抛出 404 | 数据库查询 |
| `backend/api/routes/knowledge_base.py:81` | 更新知识库 | 抛出 404 | 数据库更新 |
| `backend/api/routes/knowledge_base.py:88` | 删除知识库 | 伪删除 | 删除记录+清理资源 |
| `backend/api/routes/config.py:65` | 获取活跃配置 | 硬编码返回 | 从数据库读取 |
| `backend/api/routes/config.py:77` | 更新活跃配置 | 仅日志 | 持久化到数据库 |

### 4.3 向量存储服务

| 文件位置 | 功能 | 当前状态 | 备注 |
|----------|------|----------|------|
| `backend/services/stores/milvus_store.py:77` | add_documents | NotImplementedError | 需实现 |
| `backend/services/stores/milvus_store.py:86` | similarity_search | NotImplementedError | 需实现 |
| `backend/services/stores/milvus_store.py:90` | delete_documents | NotImplementedError | 需实现 |
| `backend/services/stores/milvus_store.py:94` | delete_collection | NotImplementedError | 需实现 |
| `backend/services/stores/faiss_store.py:82` | add_documents | NotImplementedError | 需实现 |
| `backend/services/stores/faiss_store.py:91` | similarity_search | NotImplementedError | 需实现 |
| `backend/services/stores/faiss_store.py:95` | delete_documents | NotImplementedError | FAISS 不支持删除 |
...
3. **知识库 CRUD** (`backend/api/routes/knowledge_base.py`)
   - 连接数据库实现所有操作
